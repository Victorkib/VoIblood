...
import { sendDonorStatusEmail, sendDonorThankYouEmail, sendPostDonationHealthEmail } from '@/lib/email-service'
import { isConfirmedBloodType } from '@/lib/donor-blood-types'
 ...
 */
export async function sendDonorStatusNotification(donor, drive, newStatus, metadata = {}) {
  const notifications = []
  ...
  switch (newStatus) {
    ...
    case 'completed':
      ...
      const thanksSMS = await sendThankYouSMS(donor, drive, metadata)
      const thanksEmail = await sendThankYouEmail(donor, drive, metadata)
 ...
 */
async function sendThankYouSMS(donor, drive, metadata = {}) {
  const nextEligible = metadata.nextEligibleDate
    ? new Date(metadata.nextEligibleDate)
    : (() => {
        const d = new Date()
  ...
  const bloodType =
    metadata.bloodType || (isConfirmedBloodType(donor.bloodType) ? donor.bloodType : null)
  const bloodTypeNote = bloodType
    ? metadata.bloodTypeNewlyConfirmed
      ? ` Your confirmed blood type: ${bloodType}.`
      : ` Blood type on file: ${bloodType}.`
    : ''

  const message = `🎉 AMAZING ${donor.firstName}! Your donation will save up to 3 lives! You're a true hero!${bloodTypeNote} Next eligible donation date: ${nextDate}. See you then! ❤️🩸`
 ...
 */
async function sendThankYouEmail(donor, drive, metadata = {}) {
  if (!donor?.email) {
    return { success: false, error: 'Donor email missing' }
  }
  ...
  const gratitude = metadata.gratitudePoints || null
  const bloodType =
    metadata.bloodType || (isConfirmedBloodType(donor.bloodType) ? donor.bloodType : null)
  const bloodTypeNewlyConfirmed = Boolean(metadata.bloodTypeNewlyConfirmed && bloodType)

  const thankYouResult = await sendDonorThankYouEmail({
    to: donor.email,
    donorName: `${donor.firstName} ${donor.lastName}`,
    ...
    profileUrl,
    gratitudePointsAwarded: gratitude?.awarded ? gratitude.points : 0,
    gratitudePointsBalance: gratitude?.awarded ? gratitude.balance : null,
    bloodType,
    bloodTypeNewlyConfirmed,
  })

  // Send detailed post-donation blood-work update as a second email.
  // This gives donors clinical follow-up beyond the thank-you message.
  const healthUpdateResult = await sendPostDonationHealthEmail({
    to: donor.email,
    donorName: `${donor.firstName} ${donor.lastName}`,
    subject: `🧪 Your Donation Health Update - ${drive.name}`,
    driveName: drive.name,
    donationDate: metadata.donationDate || new Date(),
    nextEligibleDate: nextDate,
    bloodWorkStatus: metadata.eligibilityStatus || 'pending',
    bloodWorkFindings: metadata.bloodWorkFindings || '',
  ...
  return {
    success: Boolean(thankYouResult?.success || healthUpdateResult?.success),
    details: {
      thankYou: thankYouResult,
      healthUpdate: healthUpdateResult,
    },
  }
}
