...
/**
 * Drive supporter registration — share-focused welcome (not in donation queue).
 */
export async function sendSupporterDriveRegistrationEmail(options) {
  const { to, supporterName, driveName, shareUrl, organizationName, nextEligibleDisplay } = options
  const subject = `You're a drive supporter — ${driveName || 'Blood donation drive'}`
  const html = `
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"/></head>
<body style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;background:#fff7ed;">
  <div style="background:#fff;border-radius:12px;padding:28px;border:1px solid #fed7aa;">
    <h1 style="color:#c2410c;margin:0 0 12px;font-size:22px;">Thank you for supporting our drive</h1>
    <p style="color:#374151;line-height:1.6;">Dear ${supporterName},</p>
    <p style="color:#374151;line-height:1.6;">You're registered as a <strong>drive supporter</strong> for <strong>${driveName || 'our blood drive'}</strong>${organizationName ? ` with ${organizationName}` : ''}. You won't be in the donation queue — your role is to spread the word and help eligible friends register.</p>
    ${
      nextEligibleDisplay
        ? `<p style="color:#92400e;line-height:1.6;background:#fffbeb;padding:12px;border-radius:8px;border-left:4px solid #f59e0b;">When you're ready to donate again, your next eligible date on file is <strong>${nextEligibleDisplay}</strong>.</p>`
        : ''
    }
    <p style="color:#374151;line-height:1.6;"><strong>Share this registration link:</strong></p>
    <p style="text-align:center;margin:20px 0;">
      <a href="${shareUrl}" style="display:inline-block;background:#ea580c;color:#fff;padding:14px 24px;border-radius:8px;text-decoration:none;font-weight:bold;">Share the drive</a>
    </p>
    <p style="font-size:12px;color:#6b7280;word-break:break-all;">${shareUrl}</p>
    <p style="font-size:12px;color:#6b7280;margin-top:20px;">Every share helps save lives. Thank you for being part of the solution.</p>
  </div>
</body>
</html>`
  const text = `Dear ${supporterName},\n\nYou're a drive supporter for ${driveName || 'a blood drive'}.\n\nShare link: ${shareUrl}\n\niBlood`
  return sendEmail({ to, subject, html, text })
}
