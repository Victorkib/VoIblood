...
const REQUEST_STATUS = ['pending', 'approved', 'rejected', 'partially_fulfilled', 'ready_for_delivery', 'fulfilled', 'cancelled']
...
// Instance method to mark as ready for delivery (units packed, awaiting handoff)
requestSchema.methods.markReadyForDelivery = async function () {
  if (this.allocatedUnits.length === 0) {
    throw new Error('No units allocated for this request')
  }
  if (!['approved', 'partially_fulfilled', 'pending'].includes(this.status)) {
    throw new Error(`Request cannot be prepared for delivery from status "${this.status}"`)
  }
