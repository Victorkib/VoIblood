...
const donorSchema = new mongoose.Schema(
  {
    ...
    /** Digits-only phone for duplicate detection (set in pre-validate). */
    phoneNormalized: {
      type: String,
      default: '',
      trim: true,
    ...
    donationHistory: [{
      ...
      /** whole_blood | rbc | plasma | platelets | cryo */
      component: {
        type: String,
        enum: ['whole_blood', 'rbc', 'plasma', 'platelets', 'cryo'],
        default: 'whole_blood',
      },
      /** drive = blood drive event; facility = in-house / inventory entry */
      collectionType: {
        type: String,
        enum: ['drive', 'facility', 'walk_in'],
        default: 'drive',
      },
      ...
      eligibilityStatus: {
        type: String,
        enum: ['eligible', 'temporarily_deferred', 'ineligible', 'pending'],
        default: 'pending',
      },
      bloodWorkSummary: {
        type: String,
    ...
    /** Platform gratitude wallet (cross-org redemption in Kenya). */
    walletId: {
      type: mongoose.Schema.Types.ObjectId,
      ref: 'DonorWallet',
      index: true,
    ...
    /** SHA-256 of normalized national ID — never store raw ID. */
    nationalIdHash: {
      type: String,
      trim: true,
      sparse: true,
    ...
    registrationType: {
      type: String,
      enum: ['online', 'walk_in', 'admin_quick'],
      default: 'online',
    },

    /** One row per drive outreach batch (activation) to avoid duplicate blasts. */
    driveOutreachHistory: [
      {
        driveId: { type: mongoose.Schema.Types.ObjectId, ref: 'DonationDrive', required: true },
        eligibleVariant: { type: Boolean, required: true },
...
donorSchema.index({ organizationId: 1, email: 1 })
donorSchema.index({ organizationId: 1, phoneNormalized: 1 })

// Phone normalization on save — avoid pre('validate') with Mongoose 9 (can throw "next is not a function").
donorSchema.pre('save', function () {
  if (this.phone) {
    this.phoneNormalized = normalizePhoneDigits(this.phone)
  } else {
