...
async function sendViaMailjet(options) {
  ...
  if (!fromEmail) {
    throw new Error('No sender email configured (MAILJET_FROM_EMAIL or GMAIL_USER)')
  }
    ...
    Messages: [
      {
        From: {
          Email: fromEmail,
          Name: options.fromName || process.env.MAILJET_FROM_NAME || 'iBlood System',
        },
        To: [
          {
            Email: options.to,
...
export async function sendEmail(options) {
  ...
  const emailOptions = {
    from: from || process.env.GMAIL_USER || process.env.MAILJET_FROM_EMAIL,
    fromName: fromName || process.env.MAILJET_FROM_NAME || 'iBlood System',
    to,
    subject,
    text,
    html: html || text,
...
export function getEmailServiceStatus() {
  const hasGmail = !!(
    process.env.GMAIL_USER && String(process.env.GMAIL_APP_PASSWORD || '').replace(/\s/g, '')
  )
  const hasMailjet = !!(process.env.MAILJET_API_KEY && process.env.MAILJET_SECRET_KEY)
  const mailjetFrom = process.env.MAILJET_FROM_EMAIL || process.env.GMAIL_USER || null
...
export async function sendDonorRegistrationEmail(options) {
  const { to, donorName, driveName, appointmentDate, donorToken } = options
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'
  const profileUrl = donorToken ? `${appUrl}/donor/${donorToken}` : null
  ...
  const html = `
    ...
    <body>
      ...
      <div class="content">
        ...
        ${
          profileUrl
            ? `<p style="text-align:center;"><a href="${profileUrl}" class="button">View your donor profile</a></p>`
            : '<p class="muted">Your organizer will share your personal donor profile link with you.</p>'
        }
      </div>
      <div class="footer">
        <p>iBlood Blood Donation System</p>
  ...
  const text = [
    ...
    `Your registration for ${driveName || 'Blood Donation Drive'} is confirmed.`,
    appointmentDate ? `Date: ${new Date(appointmentDate).toLocaleDateString()}` : '',
...
export async function sendDonorThankYouEmail(options) {
  const {
    ...
    impactMessage,
    profileUrl,
    gratitudePointsAwarded = 0,
    gratitudePointsBalance = null,
    bloodType = null,
    bloodTypeNewlyConfirmed = false,
  } = options

  const bloodTypeBlock =
    bloodType && bloodTypeNewlyConfirmed
      ? `
        <div style="background:#eff6ff;padding:20px;border-radius:8px;margin:20px 0;border-left:4px solid #2563eb;text-align:center;">
          <p style="margin:0 0 8px;font-weight:bold;color:#1e40af;">Your confirmed blood type</p>
          <p style="margin:0;font-size:28px;font-weight:bold;color:#1d4ed8;">${bloodType}</p>
          <p style="margin:10px 0 0;font-size:13px;color:#374151;">Confirmed during screening at this donation. Keep this for your records.</p>
        </div>`
      : bloodType
        ? `
        <div style="background:#f3f4f6;padding:16px;border-radius:8px;margin:20px 0;text-align:center;">
          <p style="margin:0;font-size:14px;color:#374151;">Blood type on file: <strong style="font-size:20px;color:#dc2626;">${bloodType}</strong></p>
        </div>`
        : ''

  const gratitudeBlock =
    gratitudePointsAwarded > 0 && profileUrl
      ? `
        <div style="background:#fffbeb;padding:20px;border-radius:8px;margin:20px 0;border-left:4px solid #f59e0b;">
          <p style="margin:0 0 8px;font-weight:bold;color:#92400e;">Gratitude Points (thank-you benefits)</p>
          <p style="margin:0;color:#374151;">You earned <strong>${gratitudePointsAwarded}</strong> points. Balance: <strong>${gratitudePointsBalance ?? gratitudePointsAwarded}</strong>. Not payment for blood — redeem at partner hospitals in Kenya with your ID in person.</p>
          <p style="margin:12px 0 0;"><a href="${profileUrl}" style="color:#b45309;font-weight:bold;">View your donor profile &amp; points</a></p>
        </div>`
      : profileUrl
        ? `<p style="text-align:center;margin:20px 0;"><a href="${profileUrl}" style="display:inline-block;padding:12px 24px;background:#dc2626;color:white;text-decoration:none;border-radius:6px;font-weight:bold;">View your donor profile</a></p>`
        : ''
  ...
  const text = `
    ...
    ${
      bloodType && bloodTypeNewlyConfirmed
        ? `\nYour confirmed blood type: ${bloodType} (confirmed during screening at this donation).\n`
        : bloodType
          ? `\nBlood type on file: ${bloodType}\n`
          : ''
    }
    Your Next Eligible Donation Date: ${nextEligibleDate}
    ${
      gratitudePointsAwarded > 0
        ? `\nGratitude Points: +${gratitudePointsAwarded} (balance ${gratitudePointsBalance ?? gratitudePointsAwarded}). Thank-you benefits at partner hospitals — not payment for blood.\n`
        : ''
    }
    ${profileUrl ? `\nView your donor profile: ${profileUrl}\n` : ''}
  ...
  const html = `
    ...
    <body>
      ...
      <div class="content">
        ...
        ${
          bloodType
            ? `<div class="card" style="${bloodTypeNewlyConfirmed ? 'background:#eff6ff;border-left:4px solid #2563eb;' : ''}">
          <p><strong>${bloodTypeNewlyConfirmed ? 'Your confirmed blood type' : 'Blood type on file'}:</strong></p>
          <p style="font-size:22px;font-weight:bold;margin:8px 0 0;color:#1d4ed8;">${bloodType}</p>
          ${
            bloodTypeNewlyConfirmed
              ? '<p class="muted" style="margin:8px 0 0;">Confirmed during screening at this donation.</p>'
              : ''
          }
        </div>`
            : ''
        ...
        ${
          gratitudePointsBalance != null && profileUrl
            ? `<div class="card" style="background:#fffbeb;border-left:4px solid #f59e0b;"><p style="margin:0;"><strong>Gratitude Points:</strong> ${gratitudePointsBalance} available. Community thank-you benefits at partner hospitals (Kenya). Not payment for blood.</p></div>`
            : ''
        }
        ${
          profileUrl
            ? `<p style="text-align:center;margin:22px 0;"><a href="${profileUrl}" style="display:inline-block;padding:12px 22px;background:#dc2626;color:#fff;text-decoration:none;border-radius:6px;font-weight:bold;">Open your donor profile</a></p>`
            : ''
        }
...
${
  bloodType
    ? bloodTypeNewlyConfirmed
      ? `\nConfirmed blood type: ${bloodType} (confirmed during screening)\n`
      : `\nBlood type on file: ${bloodType}\n`
    : ''
}
Blood Work Findings:
...
${gratitudePointsBalance != null ? `Gratitude Points balance: ${gratitudePointsBalance}` : ''}
${profileUrl ? `Donor profile: ${profileUrl}` : ''}
  `.trim()
...
export function buildDriveActivationSupporterOutreachContent(options) {
  ...
  const where = `${location || 'TBD'}${city ? `, ${city}` : ''}`
  const orgLine = organizationName ? ` with ${organizationName}` : ''
  const shareUrl = publicRegistrationUrl || registrationUrl
  ...
  const text = [
    ...
    'Join as supporter:',
    registrationUrl,
    ...(shareUrl && shareUrl !== registrationUrl ? ['', 'Public link to share with eligible donors:', shareUrl] : []),
...
<body>
  <div class="wrap">
    ...
    <div class="content">
      ...
      <div class="cta-wrap">
        <a class="cta" href="${escapeHtmlForEmail(registrationUrl)}">Join as drive supporter</a>
      </div>
      ${
        shareUrl && shareUrl !== registrationUrl
          ? `<p class="secondary" style="margin-top:14px;">Share with eligible donors: <a href="${escapeHtmlForEmail(shareUrl)}">Public registration link</a></p>`
          : ''
      }
