...
export async function POST(request) {
  try {
    ...
    // Auto-calculate expiryDate if not provided (35 days from collection)
    let finalExpiryDate = expiryDate
    if (!finalExpiryDate && collectionDate) {
      ...
      finalExpiryDate = expiryDateObj.toISOString()
    }

    // Validate required fields
    if (!organizationId || !bloodType || !collectionDate) {
      return NextResponse.json(
        { error: 'Missing required fields: organizationId, bloodType, and collectionDate are required' },
        { status: 400 }
      )
    }
    ...
    if (!finalExpiryDate) {
      return NextResponse.json(
        { error: 'Expiry date is required (provide it or ensure collectionDate is valid)' },
        { status: 400 }
      )
    }
