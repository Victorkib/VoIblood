 ...
 * Handle OAuth callback from Supabase (server-side)
 */
...
export async function GET(request) {
  try {
    ...
    if (!mongoUser) {
      // Extract organization info from OAuth metadata
      const orgName = user.user_metadata?.organization_name
      const orgType = user.user_metadata?.organization_type || 'blood_bank'
      const orgEmail = user.user_metadata?.organization_email || user.email
      const orgPhone = user.user_metadata?.organization_phone || ''
      
      let organization = null
      
      // Try to find existing organization by name or email
      if (orgName) {
      ...
      // Create organization if it doesn't exist and user is admin
      if (!organization && orgName && (user.user_metadata?.role === 'admin')) {
        organization = await Organization.create({
          name: orgName,
          type: orgType,
          email: orgEmail.toLowerCase(),
          phone: orgPhone,
          address: user.user_metadata?.organization_address || '',
          city: user.user_metadata?.organization_city || '',
          state: user.user_metadata?.organization_state || '',
          country: user.user_metadata?.organization_country || '',
          registrationNumber: user.user_metadata?.organization_registration || '',
      ...
      // Update organization's createdBy field
      if (organization) {
